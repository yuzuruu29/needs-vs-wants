...
> **Vault-write safety (learned 2026-08-07):** several vault notes (Tasks.md, Decisions.md, Summary.md) contain legacy non-UTF-8 bytes from an old import. Python writes must round-trip `encoding="latin-1"` to preserve those bytes — but **NEVER open the target in `"w"` mode before the payload is known-good**: `io.open(p, "w").write(s)` truncates the file first, and a `UnicodeEncodeError` mid-write leaves it at 0 bytes (this destroyed Summary.md once; it was rebuilt from session transcripts). Always write to a temp file, then `os.replace(tmp, p)` (atomic), and prefer ASCII-only text in inserted entries. Also prefer full-line anchors in `replace()` — substring anchors have left dangling tails twice.
